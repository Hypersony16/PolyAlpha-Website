...
def _market_link(slug: str, title: str = "") -> str:
    """Safe Polymarket link. Some API rows expose market slugs, not event slugs;
    direct /event/<market_slug> can 404. Search links never dead-end.
    """
...
def wallet_payload(address: str) -> dict:
    ...
    # Keep portfolio analytics focused on live exposure. Data API can include dust,
    # redeemed/closed or settled rows; those created confusing “open position” totals.
    now = datetime.now(timezone.utc)
    active_rows = []
    for r in rows:
        ...
        if close:
            try:
                is_future = datetime.fromisoformat(close.replace("Z", "+00:00")) > now
            except Exception:
                is_future = True
        if r.get("value", 0) >= 1 and r.get("size", 0) > 0 and is_future:
