...
from bot.web_alerts import ensure_web_alert_tables, telegram_ready
...
def notification_payload(qs: dict | None = None, save: bool = False) -> dict:
    """Read or update Telegram notification settings. Updates are POST-only."""
    qs = qs or {}; ensure_alpha_tables(); ensure_v4_tables(); ensure_web_alert_tables()
    conn=get_conn(); cur=conn.cursor()
    def qval(name, default=''):
        value=qs.get(name,default); return value[0] if isinstance(value,list) else value
    chat_id=str(qval('chat_id','') or '').strip()
    ...
    except Exception: min_edge=.08
    if save:
        cur.execute("""INSERT INTO web_notification_settings(id,chat_id,min_alpha,min_edge,enabled,updated_at)
        VALUES(1,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET chat_id=excluded.chat_id,min_alpha=excluded.min_alpha,
        min_edge=excluded.min_edge,enabled=excluded.enabled,updated_at=excluded.updated_at""",
        (chat_id,min_alpha,min_edge,enabled,_now_iso())); conn.commit()
    row=cur.execute('SELECT chat_id,min_alpha,min_edge,enabled,updated_at FROM web_notification_settings WHERE id=1').fetchone()
    deliveries=[]
    try:
        cols=['chat_id','status','error','created_at']; deliveries=[dict(zip(cols,r)) for r in cur.execute('SELECT chat_id,status,error,created_at FROM web_telegram_deliveries ORDER BY id DESC LIMIT 10').fetchall()]
    except Exception: pass
    conn.close()
    settings={'chat_id':row[0],'min_alpha':row[1],'min_edge':row[2],'enabled':bool(row[3]),'updated_at':row[4]} if row else {'chat_id':'','min_alpha':80,'min_edge':.08,'enabled':False}
    return {'ok':True,'settings':settings,'telegram_ready':telegram_ready(),'deliveries':deliveries}
...
def scan_payload(limit: int = 100) -> dict:
    from bot.smart_money import SmartMoneyEngine
    ensure_alpha_tables(); ensure_v4_tables()
    limit = max(25, min(300, int(limit or 100)))
    res = _run_async(SmartMoneyEngine().discover_from_leaderboards("OVERALL", "MONTH", "PNL", limit=limit, score_top=min(limit,200)))
...
def health_payload() -> dict:
    """Fast operational health/freshness snapshot with no external API calls."""
    started = time.perf_counter()
    ensure_alpha_tables(); ensure_v4_tables()
    conn = get_conn()
    latest = {}
    checks = {
        "scan": ("alpha_scan_runs", "created_at"),
        "signal": ("consensus_signals", "created_at"),
        "whale": ("whale_alerts", "created_at"),
        "wallet_event": ("wallet_position_events", "created_at"),
    }
    for key, (table, col) in checks.items():
        try:
            row = conn.execute(f"SELECT MAX({col}) FROM {table}").fetchone()
            latest[key] = row[0] if row and row[0] else ""
        except Exception:
            latest[key] = ""
    try:
        db_size = Path(os.getenv("DB_PATH", str(ROOT / "polyalpha.db"))).stat().st_size
    except Exception:
        db_size = 0
    conn.close()
    newest = max((x for x in latest.values() if x), default="")
    stale_seconds = None
    if newest:
        try:
            parsed = datetime.fromisoformat(str(newest).replace("Z", "+00:00"))
            if parsed.tzinfo is None: parsed = parsed.replace(tzinfo=timezone.utc)
            stale_seconds = max(0, int((datetime.now(timezone.utc) - parsed).total_seconds()))
        except Exception:
            pass
    return {
        "ok": True, "status": "healthy", "server_time": _now_iso(),
        "latest": latest, "freshness_seconds": stale_seconds,
        "db_size_bytes": db_size, "telegram_ready": telegram_ready(),
        "latency_ms": round((time.perf_counter()-started)*1000, 1),
        "version": Handler.server_version if "Handler" in globals() else "PolyAlpha/4.8",
    }



def arbitrage_payload() -> dict:
    """Find same-event binary outcome pairs whose combined cached prices are below $1.
    This is a conservative market-data scanner, not a fill guarantee.
    """
    import re
    rows = latest_consensus(500) or []
    groups = defaultdict(list)

    def sport_of(text: str) -> str:
        t=(text or '').lower()
        if re.search(r'ufc|mma|fight|bout', t): return 'UFC'
        if re.search(r'tennis|atp|wta|set betting', t): return 'Tennis'
        if re.search(r'basketball|nba|wnba|ncaa|euroleague', t): return 'Basketball'
        if re.search(r'soccer|football|fifa|premier league|champions league|la liga|serie a|bundesliga|spread|o/u|total', t): return 'Soccer'
        return ''

    def key_for(row: dict) -> str:
        title=str(row.get('title') or row.get('market') or '')
        slug=str(row.get('market') or row.get('slug') or '')
        base=re.sub(r'-(yes|no|up|down|over|under|home|away)$','',slug.lower())
        base=re.sub(r'\b(yes|no|up|down|over|under)\b','',base)
        return re.sub(r'[^a-z0-9]+','-',base).strip('-') or re.sub(r'[^a-z0-9]+','-',title.lower()).strip('-')

    for row in rows:
        title=str(row.get('title') or row.get('market') or '')
        sport=sport_of(title+' '+str(row.get('market') or ''))
        if not sport: continue
        try: price=float(row.get('avg_price') or row.get('price') or 0)
        except Exception: continue
        if price <= 0 or price >= 1: continue
        groups[key_for(row)].append((row,price,sport))

    opportunities=[]
    opposites={('yes','no'),('no','yes'),('up','down'),('down','up'),('over','under'),('under','over'),('home','away'),('away','home')}
    for key, items in groups.items():
        if len(items)<2: continue
        best=None
        for i,(a,pa,sport) in enumerate(items):
            oa=str(a.get('outcome') or '').lower().strip()
            for b,pb,_ in items[i+1:]:
                ob=str(b.get('outcome') or '').lower().strip()
                if (oa,ob) not in opposites and oa==ob: continue
                total=pa+pb
                if total>=1 or total<=0: continue
                profit=(1/total-1)*100
                candidate={
                    'title': a.get('title') or b.get('title') or key,
                    'event_key': key, 'sport': sport, 'total_cost': round(total,4),
                    'profit_pct': round(profit,3), 'slug': a.get('market') or a.get('slug') or '',
                    'legs':[{'outcome':a.get('outcome') or 'Leg A','price':round(pa,4),'slug':a.get('market') or ''},
                            {'outcome':b.get('outcome') or 'Leg B','price':round(pb,4),'slug':b.get('market') or ''}],
                    'closes_at': a.get('closes_at') or _parse_market_close(a.get('market',''), a.get('title',''))
                }
                if best is None or candidate['profit_pct']>best['profit_pct']: best=candidate
        if best: opportunities.append(best)
    opportunities.sort(key=lambda x:x['profit_pct'], reverse=True)
    return {'ok':True,'opportunities':opportunities[:100],'scanned_signals':len(rows),'server_time':_now_iso(),
            'disclaimer':'Prices are cached indications. Verify executable order-book depth and fees before trading.'}

def route_api(path: str, qs: dict, method: str = "GET", body: dict | None = None) -> tuple[int, dict]:
    try:
        body = body or {}
        if path == "/api/health": return 200, health_payload()
        ...
        if path == "/api/arbitrage": return 200, arbitrage_payload()
